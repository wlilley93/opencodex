...
export const GUARDS: Guard[] = [
  ...
  {
    name: "an unparseable speech body is reported as such",
    file: "src/server/audio-speech.ts",
    from: '    return invalid("Speech request body is not valid JSON");',
    to: '    return invalid("Speech request body must be a JSON object");',
    expect: "refuses a body that is not valid JSON",
    suite: VOICE,
  },
  {
    name: "a speech body that fails mid-read is reported as malformed",
    file: "src/server/audio-speech.ts",
    from: '      : invalid("Malformed speech request body");',
    to: '      : invalid("Speech request body is not valid JSON");',
    expect: "refuses a body stream that fails mid-read",
    suite: VOICE,
  },
  {
    name: "a nonempty speech input is required",
    file: "src/server/audio-speech.ts",
    from: '  if (typeof body.input !== "string" || body.input.trim() === "") {',
    to: "  if (false) {",
    expect: "refuses no input at all",
    suite: VOICE,
  },
  {
    name: "a malformed multipart body is reported as such",
    file: "src/server/audio-transcriptions.ts",
    from: '      : invalid("Malformed audio multipart body");',
    to: '      : invalid("A nonempty audio file is required");',
    expect: "refuses a multipart body whose boundary does not match",
    suite: VOICE,
  },
  {
    name: "a repeated transcription field is refused",
    file: "src/server/audio-transcriptions.ts",
    from: "    if (seen.has(name)) return invalid(",
    to: "    if (false) return invalid(",
    expect: "refuses the same field twice",
    suite: VOICE,
  },
  {
    name: "a nonempty audio file is required",
    file: "src/server/audio-transcriptions.ts",
    from: '  if (!(file instanceof File) || file.size === 0) return invalid("A nonempty audio file is required");',
    to: "",
